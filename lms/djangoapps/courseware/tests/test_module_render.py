...
class GradedStatelessXBlock(XBlock):
    ...
    @XBlock.json_handler
    def set_score(self, json_data, suffix):
...
class TestHandleXBlockCallback(ModuleStoreTestCase, LoginEnrollmentTestCase):
    ...
    def test_score_without_student_state(self):
        course = CourseFactory()
        block = ItemFactory(category='stateless_scorer', parent=course)

        request = self.request_factory.post('dummy_url', data=json.dumps({"grade": 0.75}), content_type='application/json')
        request.user = self.mock_user
